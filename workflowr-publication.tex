...
\affil[1]{Department of Human Genetics, University of Chicago, Chicago,
IL, 60637, USA}

\affil[2]{Research Computing Center, University of Chicago, Chicago, IL,
60637, USA}

\affil[3]{Department of Statistics, University of Chicago, Chicago, IL,
60637, USA}
...
a static website---thereby conveniently shareable with collaborators by
sending them a URL---and accompanied by source code and reproducibility
safeguards. The \textbf{workflowr} R package is open source and available on
...
programming---the publishing of text and code in a readable form---and
more closely integrates other features such as tracking project
...
% Figure \ref{fig:start}

\centering

\includegraphics[width=0.8\textwidth]{figures/start.eps}

\cprotect\caption{\label{fig:start}
...
files as the project progresses---as the project develops, additional
Rmd files may be organized in the "analyses" folder. This is only a
...
into the project---\texttt{wflow\_start()} also easily accommodates this scenario,
with additional arguments to control how the \textbf{workflowr} files are added
to the existing project. See the package vignette, ``Migrating an
existing project to use \textbf{workflowr},'' for more details; it can be
...
\includegraphics[width=0.8\textwidth]{figures/build.eps}

\cprotect\caption{\label{fig:build}
...
\includegraphics[width=0.8\textwidth]{figures/publish.eps}
...
a three-step procedure to store the code and results in a project
...
Figure \ref{fig:status}. The “published” Rmd files, which are those that have been run
...
\includegraphics[width=0.8\textwidth]{figures/status.eps}

\cprotect\caption{\label{fig:status}
...
use of \textbf{workflowr} as a scientific notebook---the webpages contain written
notes, mathematical equations, source code, and the outputs generated
