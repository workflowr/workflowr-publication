...
simply following the ``workflowr workflow'', R users can create projects
...
needed to begin a \textbf{workflowr} project (files and directories created by
\texttt{wflow\_start()} are in red). The default directory structure encourages
...
directories.) Files required by \textbf{workflowr} are depicted in boldface. B)
All results are organized into a website (stored in \texttt{docs/}. The
use of hyperlinks allows for efficient access to the results. The
screenshots above illustrate how a \textbf{workflowr} website can be navigated.
Clicking a hyperlink in the main page, \texttt{index.html}, (1)
navigates the browser to a webpage containing some results,
\texttt{visualize.html}; clicking on the ``Home'' hyperlink (2) in the
navigation bar brings the browser back to the main page. For larger
...
\texttt{render()} command from the \textbf{rmarkdown} package with several reproducibility
safeguards:
